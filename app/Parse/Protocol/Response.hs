...
module Parse.Protocol.Response where
...
data Packet = Packet { header :: Header
                     , body   :: B.ByteString -- [B.ByteString]
                     } deriving (Show, Eq)

pPacket :: Parser Packet
pPacket = do
  head' <- pHeader
  body' <- takeByteString -- Consumes all -- Temporary bad solutions
...
getResponseEx = C8.readFile "app/data/response.ex"
